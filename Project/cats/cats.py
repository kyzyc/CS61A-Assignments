...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def process(s):
        trans_s = split(lower(remove_punctuation(s)))
        for word in trans_s:
            if word in subject:
                return True
        return False
...
def accuracy(typed, source):
    ...
    if len_typed == 0 or len_source == 0:
        return 100.0 if len_typed == len_source else 0.0
    ...
    return (matched / len_typed) * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    diffs = [(diff_function(typed_word, word, limit), word) for word in word_list]
    
    diff, word = min(diffs, key=lambda x: abs(x[0]))

    return typed_word if diff > limit else word
    # END PROBLEM 5
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    upload({'id': user_id, 'progress': matched / len(source)})
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    f = lambda tts: [(tts[i] - tts[i - 1]) for i in range(1, len(tts))]
    times = [f(tts) for tts in tpp]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    time_player_pair_per_word = [[(get_time(times, player_indice, word_indice), player_indice) \
     for player_indice in player_indices] for word_indice in word_indices]
    fastest_player_per_word = [min(time_player_pair, key = lambda x:x[0])[1] \
                               for time_player_pair in time_player_pair_per_word]

    result = [[] for _ in player_indices]

    for word_index, fastest_player in enumerate(fastest_player_per_word):
