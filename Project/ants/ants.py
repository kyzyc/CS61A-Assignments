    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.is_container:
                assert not place.ant.is_container, 'Too many ants in {0}'.format(place)
                self.store_ant(place.ant)
                place.ant = self
            else:
                assert place.ant.is_container and place.ant.can_contain(self), 'Too many ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not cur_place.is_hive and (len(cur_place.bees) == 0 or cnt < self.lower_bound):
            cur_place = cur_place.entrance
            cnt += 1
        
        if cur_place.is_hive or len(cur_place.bees) == 0 or cnt > self.upper_bound:
            return None
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt select a random Bee and eat it."""
    name = 'Hungry'
    food_cost = 4
    chew_cooldown = 3
    lower_bount, upper_bound = 0, 0
    implemented = True
    ...
    def action(self, gamestate):
        """Eat a bee in current place if cooldown = 0."""
        if self.cooldown != 0:
            self.cooldown -= 1
            return

        if len(self.place.bees):
            cloest_bee = random_bee(self.place.bees)
            cloest_bee.reduce_health(cloest_bee.health)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while cur_place and not cur_place.is_hive:
            ...
            if cur_place.ant and cur_place.ant != self:
                ret[cur_place.ant] = dis
                if cur_place.ant.is_container and cur_place.ant.ant_contained != self:
                    ret[cur_place.ant.ant_contained] = dis
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        d = self.damage - distance * self.distance_decay
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if 'scary_turn' in vars(self) and self.scary_turn > 0:
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if 'scary_turn' in vars(self):
            return
        self.scary_turn = length
        # END Problem EC 2
